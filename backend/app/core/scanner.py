...
async def check_single_platform(
    ...
    # Build safe URL
    try:
        url = build_safe_check_url(site.uriCheck, username, site.stripBadChar)
    ...
    # Build pretty profile URL for display
    profile_url = None
    if site.uriPretty:
        try:
            profile_url = site.uriPretty.replace("{account}", username)
        except Exception:
            profile_url = url
    else:
        profile_url = url
    ...
    async with semaphore:
        ...
        try:
            ...
            if site.postBody:
                ...
                post_payload = post_body_str.replace("{account}", username)
...
async def run_osint_orchestrator(session: ScanSession, options: Optional[dict] = None):
    ...
    async def execute_plugin(plugin):
        ...
        try:
            async for result in plugin.execute(session.target, session.input_type, options):
                if session.cancel_event.is_set():
                    break
